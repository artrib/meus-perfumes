...
def remover_acentos(texto):
    if not isinstance(texto, str):
        texto = str(texto)
    # Normaliza para remover acentos e manter apenas a base da letra
    return "".join(
...
def padronizar_texto(texto):
    """Remove acentos e coloca em formato Título (Ex: Cítrico -> Citrico)"""
    if not texto: return ""
    return remover_acentos(texto.strip()).title()
...
if choice == "🔍 Pesquisar":
    ...
    if search:
        termos = search.split()
        for termo in termos:
            t_norm = remover_acentos(termo).lower()
            mask = result.apply(
                lambda row: row.astype(str).map(lambda x: remover_acentos(x).lower()).str.contains(t_norm).any(),
                axis=1
            )
            result = result[mask].copy()

    st.write(f"{len(result)} perfumes listados")

    if not df.empty:
        # Tabela (usa result para a pesquisa)
        st.data_editor(
        ...
        # =================================================
        # GRÁFICOS (Sempre baseados no DF TOTAL)
        # =================================================
        ...
        with col1:
            ...
            c_est = c_est[c_est != ""].value_counts().reset_index(name="count")
            c_est.columns = ["Estações", "count"]
            
            fig1 = px.bar(c_est, x="Estações", y="count", text="count", color_discrete_sequence=['#B0A695'])
        ...
        # 2. NOTAS OLFATIVAS (Unificando Citricos/Cítricos)
        with col2:
            n_s = df["Notas Olfativas"].str.split(',').explode().str.strip()
            # AQUI A MÁGICA: Remove acentos e padroniza para Title Case
            n_s = n_s[n_s != ""].apply(padronizar_texto)
            
            c_not = n_s.value_counts().nlargest(30).reset_index(name="count")
            ...
            fig2.update_layout(yaxis={'categoryorder': 'total ascending'}, height=max(400, len(c_not)*22), margin=dict(t=10, b=10), xaxis_title=None, yaxis_title=None)
            st.plotly_chart(fig2, use_container_width=True, config=config_fixo)

        col3, col4 = st.columns(2)

        # 3. FAMÍLIA OLFATIVA (Unificando também)
        with col3:
            f_s = df["Família Olfativa"].str.split('/').explode().str.strip().apply(padronizar_texto)
            c_fam = f_s[f_s != ""].value_counts().nlargest(8).reset_index(name="count")
        ...
        # 4. PERFUMISTAS
        with col4:
            c_perf = df["Perfumista"].replace(["", "nan"], "Desconhecido").value_counts().nlargest(15).reset_index(name="count")
...
# =========================================================
# ADICIONAR / EDITAR / APAGAR (Mantidos conforme lógica anterior)
# =========================================================
    ...
    with st.form("add"):
        ...
        if st.form_submit_button("Guardar"):
            if nome:
                new = pd.DataFrame([{"Ano": ano, "Nome do Perfume": nome, "Estações do Ano": ", ".join(est), 
                                     "Ocasiões de Uso": ", ".join(oc), "Família Olfativa": fam, 
    ...
    if not df.empty:
        df_edit = df.copy()
        df_edit['label'] = df_edit['Nome do Perfume'] + " (" + df_edit['Marca'] + ")"
        sel = st.selectbox("Selecione:", sorted(df_edit['label'].tolist()))
        idx = df_edit[df_edit['label'] == sel].index[0]
    ...
    if not df.empty:
        df_del = df.copy()
        df_del['label'] = df_del['Nome do Perfume'] + " (" + df_del['Marca'] + ")"
        p_del = st.selectbox("Perfume:", sorted(df_del['label'].tolist()))
        if st.button("Confirmar"):
            idx_del = df_del[df_del['label'] == p_del].index[0]
            df = df.drop(idx_del)
            df.to_csv(DB_FILE, index=False, encoding='utf-8-sig')
