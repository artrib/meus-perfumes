...
if choice == "🔍 Pesquisar":
    ...
    result = df.copy()
    # Insere coluna de edição no início para a tabela
    ...
    if not df.empty:
        # Habilita a edição apenas na coluna "Editar"
        ...
        # Lógica para saltar para a aba Editar ao marcar o checkbox
...
# =========================================================
    ...
    with st.form("add"):
        ...
        if st.form_submit_button("Guardar"):
            if nome:
                ...
                new = pd.DataFrame([{"Ano": ano, "Nome do Perfume": nome, "Estações do Ano": ", ".join(est), "Ocasiões de Uso": ", ".join(oc), "Família Olfativa": fam_clean, "Notas Olfativas": notas_clean, "Marca": marca, "Perfumista": perf_clean}])
    ...
    if not df.empty:
        ...
        # Define o perfume selecionado (vido da pesquisa ou manual)
        idx_default = 0
        ...
        # Botão para limpar a seleção vinda da pesquisa
        ...
        with st.form("edit"):
            ...
            if st.form_submit_button("Atualizar"):
                ...
                df.to_csv(DB_FILE, index=False, encoding='utf-8-sig')
                st.session_state.edit_perfume = None # Limpa a seleção após editar
