...
# =========================================================
# GESTÃO DE ESTADO (Inicialização)
# =========================================================
...
def padronizar_texto(texto):
    if not texto or not isinstance(texto, str):
        return ""
    return texto.strip().upper() # Alterado para UPPER para bater com as constantes globais

# Caching para melhorar a performance da App
def load_data():
...
# Lógica de verificação do index do menu corretiva
if st.session_state.menu_choice in menu:
    default_index = menu.index(st.session_state.menu_choice)
    st.session_state.menu_choice = None # Limpa após usar
elif st.session_state.edit_perfume:
    default_index = 2 # Vai para Editar
else:
    default_index = 0
...
# =========================================================
        ...
        with col1:
            # GRÁFICO 1: ESTAÇÕES (Corrigido para bater com UPPERCASE)
            ...
            fig1 = px.bar(
                c_est, x="Estações do Ano", y="count", text="count", 
                color_discrete_sequence=['#B0A695'],
                category_orders={"Estações do Ano": ESTACOES_LISTA}
            )
            ...
            fig5 = px.bar(
                c_oc, x="Ocasiões", y="count", text="count", 
                color_discrete_sequence=['#C08261'],
                category_orders={"Ocasiões": OCASIOES_OPCOES}
            )
    ...
    with st.form("add"):
        ...
        if st.form_submit_button("Guardar"):
            if not nome:
                ...
                new = pd.DataFrame([{
                    "Ano": ano, "Nome do Perfume": nome.strip(), "Estações do Ano": ", ".join(est),
                    "Ocasiões de Uso": ", ".join(oc), "Família Olfativa": fam_clean,
                    "Notas Olfativas": notas_clean, "Marca": marca.strip(), "Perfumista": perf_clean
                }])
                
                df = pd.concat([df, new], ignore_index=True)
                df.to_csv(DB_FILE, index=False, encoding='utf-8-sig')
                
                # Correção do redirecionamento para bater exatamente com a string do menu
    ...
    if not df.empty:
        ...
        with st.form("edit"):
            ...
            if st.form_submit_button("Atualizar"):
                ...
                df.loc[idx] = [e_a, e_n.strip(), ", ".join(e_e), ", ".join(e_oc), fam_edit, notas_edit, e_m.strip(), padronizar_texto(e_p)]
                df.to_csv(DB_FILE, index=False, encoding='utf-8-sig')
                
                st.session_state.edit_perfume = None
                st.session_state.menu_choice = " Pesquisar" # Redireciona após editar
