...
def load_data():
    cols_mapping = {
        "ano": "Ano",
        "nome_perfume": "Nome do Perfume",
        "estacoes_ano": "Estações do Ano",
        "ocasioes_uso": "Ocasiões de Uso",
        "familia_olfativa": "Família Olfativa",
        "notas_olfativas": "Notas Olfativas",
        "marca": "Marca",
        "perfumista": "Perfumista"
    }
    try:
        conn = sqlite3.connect(DB_NAME)
        # Seleciona as colunas e renomeia para manter compatibilidade com o código original
        query = f"SELECT {', '.join(cols_mapping.keys())} FROM perfumes"
        df = pd.read_sql_query(query, conn)
        conn.close()
        
        # Renomeia as colunas para o formato esperado pelo resto do código
        df = df.rename(columns=cols_mapping)
        
        # Garante que todas as colunas necessárias existam (mesmo que vazias)
        expected_cols = list(cols_mapping.values())
        for col in expected_cols:
            if col not in df.columns:
                df[col] = ""
        
        return df.fillna("").astype(str)[expected_cols]
...
# =========================================================
        ...
        with col1:
            ...
            df_pie = pd.DataFrame({"Periodo": ["DIA", "NOITE"], "count": [total_dia, total_noite]})
            fig_yn = px.pie(df_pie, values='count', names='Periodo', hole=0.55, color_discrete_sequence=['#9cb7ba', '#141414'])
            fig_yn.update_layout(showlegend=True, legend=dict(orientation="h", yanchor="top", y=-0.1, xanchor="center", x=0.5), margin=dict(t=10, b=50, l=10, r=10), height=300)
...
# =========================================================
        ...
        if st.form_submit_button("Guardar"):
            if not nome:
                ...
                try:
                    ...
                    cursor = conn.cursor()
                    cursor.execute('''
                        INSERT INTO perfumes (ano, nome_perfume, estacoes_ano, ocasioes_uso, familia_olfativa, notas_olfativas, marca, perfumista)
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                    ''', (ano, nome, ", ".join(est), ", ".join(oc), fam_clean, notas_clean, marca, perf_clean))
                    conn.commit()
                    conn.close()
                    ...
                    st.error("Este perfume já existe na base de dados!")
                except Exception as e:
    ...
    if not df.empty:
        ...
        # Carrega dados atuais do perfume selecionado
        perfume_data = df[df["Nome do Perfume"] == sel].iloc[0]
        
        at_oc = [x.strip() for x in str(perfume_data["Ocasiões de Uso"]).split(",") if x.strip() in OCASIOES_OPCOES]
        at_est = [x.strip() for x in str(perfume_data["Estações do Ano"]).split(",") if x.strip() in ESTACOES_LISTA]

        with st.form("edit"):
            c1, c2 = st.columns(2)
            with c1:
                e_n = st.text_input("Nome", value=perfume_data["Nome do Perfume"])
                e_m = st.text_input("Marca", value=perfume_data["Marca"])
                e_e = st.multiselect("Estações", ESTACOES_LISTA, default=at_est)
                e_oc = st.multiselect("Ocasiões", OCASIOES_OPCOES, default=at_oc)
            with c2:
                e_f = st.text_input("Família", value=perfume_data["Família Olfativa"])
                e_p = st.text_input("Perfumista", value=perfume_data["Perfumista"])
                e_a = st.text_input("Ano", value=perfume_data["Ano"])
                e_not = st.text_area("Notas", value=perfume_data["Notas Olfativas"])
            ...
            if st.form_submit_button("Atualizar"):
                ...
                try:
                    ...
                    cursor.execute('''
                        UPDATE perfumes 
                        SET ano=?, nome_perfume=?, estacoes_ano=?, ocasioes_uso=?, familia_olfativa=?, notas_olfativas=?, marca=?, perfumista=?
                        WHERE nome_perfume=?
                    ''', (e_a, e_n, ", ".join(e_e), ", ".join(e_oc), fam_edit, notas_edit, e_m, padronizar_texto(e_p), sel))
                    conn.commit()
                    conn.close()
