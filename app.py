...
if choice == "🔍 Pesquisar":
    ...
    # Texto do Total (Sem aumento de tamanho exagerado)
    ...
    if not df.empty:
        ...
        with col3:
            ...
            # Mapeamento de Cores e Legenda Normal
            cores_map = {
            ...
            fig3.update_layout(showlegend=True, height=450, margin=dict(t=10, b=10)) # Legenda normal à direita
...
# --- ABA APAGAR ---
    ...
    if not df.empty:
        p_del = st.selectbox("Escolha o perfume para apagar:", sorted(df["Nome do Perfume"].unique().tolist()))
        if st.button("Confirmar Eliminação Permanente"):
            df = df[df["Nome do Perfume"] != p_del]
            df.to_csv(DB_FILE, index=False, encoding='utf-8-sig')
            st.warning("O perfume foi removido da base de dados.")
            st.rerun()
