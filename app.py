...
if choice == " Pesquisar":
    ...
    if search:
        if local_busca == "Notas Olfativas":
            ...
            mask = result["Notas Olfativas"].apply(lambda x: t_padronizado in [padronizar_texto(n) for n in str(x).split(",")])
        ...
        if st.form_submit_button("Guardar"):
            if not nome:
                ...
                try:
                    conn = sqlite3.connect(DB_NAME)
                    cursor = conn.cursor()
                    cursor.execute("INSERT INTO perfumes (ano, nome_perfume, estacoes_ano, ocasioes_uso, familia_olfativa, notas_olfativas, marca, perfumista) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
                        (ano, nome, ", ".join(est), ", ".join(oc), fam, notas, marca, perf))
                    conn.commit(); conn.close()
                    st.toast("SALVO COM SUCESSO!"); time.sleep(1); st.rerun()
                except sqlite3.IntegrityError:
                    st.error("Este perfume já existe!")
