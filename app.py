...
if choice == "🔍 Pesquisar":
    ...
    if not df.empty:
        ...
        st.data_editor(
            ...
            column_config={
                ...
                "Estações do Ano": st.column_config.TextColumn("Estações do Ano", width=150),
                "Ocasiões de Uso": st.column_config.TextColumn("Ocasiões", width="medium"),
            }
        )
        
        # BOTÃO DE DESCARREGAMENTO CENTRALIZADO
        if not result.empty:
        ...
        with col3:
            ...
            fig3.update_layout(showlegend=True, legend=dict(orientation="h", yanchor="top", y=-0.15, xanchor="center", x=0.5), margin=dict(t=10, b=100), height=450)
        ...
        with col4:
            ...
            fig4.update_layout(yaxis={'categoryorder':'total ascending'}, height=450, margin=dict(t=10, b=10))
    ...
    with st.form("add"):
        ...
        if st.form_submit_button("Guardar"):
            if nome:
                new = pd.DataFrame([{"Ano": ano, "Nome do Perfume": nome, "Estações do Ano": ", ".join(est), "Ocasiões de Uso": ", ".join(oc), "Família Olfativa": fam, "Notas Olfativas": notas, "Marca": marca, "Perfumista": perf}])
